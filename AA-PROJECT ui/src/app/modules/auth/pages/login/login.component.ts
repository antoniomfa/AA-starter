...
export class LoginComponent implements OnInit, OnDestroy {
	 ...
	 */
	loginClick() {
		let authReq: AuthenticationRequest = new AuthenticationRequest;
		authReq.email = this.f.email.value;
		authReq.password = this.f.password.value;

		this._auth.authenticate(authReq)
			.pipe(first())
			.subscribe({
				next: () => {
					...
					this._router.navigate(['/features'], { queryParams: {} });
				},
				error: error => {
