...
namespace Identity.Services.Concrete
{
    public class AccountService : IAccountService
    {
        ...
        public async Task<BaseResponse<AuthenticationResponse>> AuthenticateAsync(AuthenticationRequest request)
        {
            ...
            if (_blacklistService.IsTokenBlacklisted(response.JWToken))
            {
                throw new ApiException($"Blacklisted token '{request.Email}'.") { StatusCode = (int)HttpStatusCode.NotAcceptable };
            }
        ...
        public async Task<BaseResponse<string>> RegisterAsync(RegisterRequest request, string uri)
        {
            ...
            if (result.Succeeded)
            {
                ...
                return new BaseResponse<string>(newUser.Id.ToString(), message: $"User Registered.");
            }
            else
            {
